...
function getScrollDistance(event) {
  ...
  const scrollBarSizeY = scrollTop  / clientHeight * 100
  const scrollX = scrollLeft / clientWidth  * 100

  return { scrollBarSizeY, scrollX } 
}
```
...
let nowBcSize = 0 // 鼠标点击时在滚动条背景中的高度
...
function clickSlideScrollbar(event) {
  ...
  scrollBarSizeY = event.clientY - event.currentTarget.getBoundingClientRect().top
...
function slideMouse(event) {
  ...
  const value = this.getScrollbar(event)

  // 拖动时 直接赋值即可
  this.$refs.wrap.scrollTop = value
}

function getScrollbar(event) {
  const wrap = this.$refs.wrap // .scrollbar__wrap 节点
  const barbcy = this.$refs.barbcy // .scrollbar__axis-y 节点
  
  // 拖动时高度 = 鼠标拖动时滚动条背景顶部距离页面顶部高度 - 滚动条背景顶部距离页面顶部高度
  nowBcSize = event.clientY - barbcy.getBoundingClientRect().top
  
  // 滚动条滚动高度 = ((拖动时高度 - 点击时高度) / 滚动条高度) * 换算百分比
  const distancebarbcy = ((nowBcSize - scrollBarSizeY) / barbcy.offsetHeight) * 100

  // 盒子内容滚动高度 = 滚动条滚动高度 * 盒子滚动区域高度 / 换算成数值
  const distanceBox = distancebarbcy * wrap.scrollHeight / 100
  return distanceBox
}
...
function clickScrollbarBC(event) {
  ...
  scrollBarSizeY = this.$refs.bary.offsetHeight / 2
  ...
  * 点击时、加载滚动动画
  *
  * 在鼠标拖动情况下，本身**scrollTop** 是逐步递进或递减的，并不会一闪而过
  * 在点击滚动条背景跳转时，会一闪而过并没有页面滚动效果，所以要在点击滚动条背景时添加页面滚动动画
  */  
  const wrap = this.$refs.wrap // .scrollbar__wrap 节点
  const length = 10 // 动画分为10步
  const speed  = Math.ceil((value - wrap.scrollTop) / length) // 每步的长度
